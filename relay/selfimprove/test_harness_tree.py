...
def test_a_branch_applies_and_says_what_it_changed():
    got = T.resolve(_tree(long_running_local={"parameters": {"max_retries": 20}}), "long_running_local")
    assert got["matched"] == "long_running_local"
...
def test_every_resolution_is_accountable():
    """説明できない設定は再現できない設定。"""
    got = T.resolve(_tree(spreadsheet={"parameters": {"memory_max_items": 2}}), "spreadsheet")
...
def test_a_branch_that_would_be_an_illegal_manifest_is_refused():
    ...
    with pytest.raises(Exception):
        T.resolve(_tree(coding={"parameters": {"max_retries": 10_000}}), "coding")
...
def test_branches_that_resolve_to_the_same_harness_are_reported_as_one():
    ...
    tree = _tree(coding={"parameters": {"memory_max_items": 9}},
                 sql={"parameters": {"memory_max_items": 9}},
                 ocr={})
    got = T.branches(tree)
...
def test_a_branch_nobody_measured_is_named_as_unjustified():
    ...
    tree = _tree(sql={"parameters": {"memory_max_items": 9}},
                 ocr={"parameters": {"memory_max_items": 2}})
    out = T.justified(tree, {"sql": ["INCONCLUSIVE", "KEEP"],
                             "ocr": ["INCONCLUSIVE"]})
    assert out["justified"] == ["sql"]
    assert out["unjustified"] == ["ocr"]
