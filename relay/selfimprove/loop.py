...
def _grade_arm(preds_dir, targets_file, dataset, run_id, max_wait_min=200):
    """Run the kiyus batch grade synchronously.
    ...
    actually ran -- kiyus unreachable, scp failed, EVALERR-everything -- i.e. an INFRA fault, NOT a
...
def validate(toggle, spec_path, n, seed, dataset_key, alpha, min_n, min_pp,
    ...
    # GRADE-INFRA GUARD: 0 real verdicts means the grade never ran (kiyus down / scp failed / EVALERR-
    # everything) -- NOT a real 0%. Gating/burning on a failed grade would record a garbage A/B and burn
    # the slice (the kiyus-down incident: ON solved 200 fine but the grade returned 0/200). Treat as infra.
    if on_graded == 0:
        log("ON grade produced 0 real verdicts -> INFRA ABORT (kiyus unreachable / scp failed); NOT gating, NOT burning")
        return {"status": "infra_abort", "arm": "ON-grade",
                "reason": "ON grade did not run (infra: kiyus unreachable / scp failed)",
                "burned": False, "report": None}
    ...
    if off_graded == 0:
        log("OFF grade produced 0 real verdicts -> INFRA ABORT (kiyus unreachable / scp failed); NOT gating, NOT burning")
        return {"status": "infra_abort", "arm": "OFF-grade",
                "reason": "OFF grade did not run (infra: kiyus unreachable / scp failed)",
                "burned": False, "report": None}
