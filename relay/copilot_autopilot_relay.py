...
_DEFAULT_DISCIPLINE = (
    ...
    # この規律は「喋りすぎ」を止めるためのもので、調べる手を止めるためのものではない。
    # 実際に、ツールを1回も叩かないまま「この環境には存在しません」と短く打ち切る挙動が
    # 出た。簡潔さは結論の出し方の話であって、確認の省略を許すものではないと明記する。
    "ただしこの規律は出力の話であり、調査の手を抜く理由にはならない。"
    "ツールの有無・実行可否は、実際に call_tool を叩いて確かめてから述べる。"
    "確かめずに「無い」「できない」と書くことは、この規律違反である。"
)
