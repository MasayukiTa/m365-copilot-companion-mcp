...
> **JTC あるある**: 「Cowork (Claude Code) は禁止になったけど Copilot ならいいでしょ」
> というロジックが通る環境なら、本ツールも通る余地はあります。ただし運用は **明示的に承認を取る**
> が原則。「禁止されていない」≠「許可されている」を取り違えると、半年後に内部監査でお呼びがかかります。
...
**Q. Cowork / Claude Code が会社で禁止になった。これは大丈夫？**
A. 別物ですが、判断は組織次第。本ツールは「Microsoft が公式に提供するチャネル（M365 Copilot）に接続するための MCP サーバー」で、Anthropic 直接 API を使うものではありません。Cowork 禁止理由が「Anthropic 直接利用の禁止」なら本ツールは抵触しません。「ローカル AI エージェント全般の禁止」なら抵触します。**自分のケースで承認を取って判断する以外ありません**。
