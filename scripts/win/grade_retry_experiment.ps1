...
$probe = & ssh -o BatchMode=yes -o ConnectTimeout=20 EVAL_HOST "echo up" 2>&1
...
& ssh -o BatchMode=yes EVAL_HOST ("powershell -NoProfile -Command New-Item -ItemType Directory -Force " + $remoteWin) 2>&1 | Out-Null
foreach ($f in @("bon_raw.jsonl", "grade_preds_1.json", "grade_preds_2.json")) {
    & scp -o BatchMode=yes "$Dir/$f" ("EVAL_HOST:" + $Remote + "/") 2>&1 | Out-Null
}
...
').write(s)"
& scp -o BatchMode=yes ".fleet/swe/retryexp_grade.sh" ("EVAL_HOST:" + $Remote + "/bon_grade.sh") 2>&1 | Out-Null
Say "inputs shipped"

$tr = '"C:\Windows\System32\wsl.exe" -d Ubuntu -e /bin/bash /mnt/c/swe-grade/retryexp/bon_grade.sh'
& ssh -o BatchMode=yes EVAL_HOST "schtasks /Delete /TN RetryExpGrade /F" 2>&1 | Out-Null
& ssh -o BatchMode=yes EVAL_HOST ('schtasks /Create /TN RetryExpGrade /TR "' + $tr.Replace('"','\"') + '" /SC ONCE /ST 23:55 /RL HIGHEST /F') 2>&1 | ForEach-Object { Say ("remote: " + $_) }
& ssh -o BatchMode=yes EVAL_HOST "schtasks /Run /TN RetryExpGrade" 2>&1 | ForEach-Object { Say ("remote: " + $_) }

Say "waiting for the verdicts"
for ($i = 0; $i -lt 120; $i++) {
    $p = & ssh -o BatchMode=yes EVAL_HOST ("if exist " + $remoteWin + "on_grade.out (findstr /C:DONE_BON_GRADE " + $remoteWin + "on_grade.out) else (echo waiting)") 2>&1
    if ($p -match "DONE_BON_GRADE") { Say "grading finished"; break }
    Start-Sleep -Seconds 30
}
foreach ($k in @("1", "2")) {
    & scp -o BatchMode=yes ("EVAL_HOST:" + $Remote + "/out_$k/eval_results.json") "$Dir/verdict_$k.json" 2>&1 | Out-Null
}
