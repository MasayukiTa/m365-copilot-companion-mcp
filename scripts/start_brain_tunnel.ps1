...
# Open the SSH REVERSE tunnel that lets the Minecraft bot on the eval host reach the local
# brain proxy WITHOUT any key ever leaving this machine:
#
#   the eval host 127.0.0.1:8012  --[ssh -R]-->  <user> 127.0.0.1:8012 (brain_proxy.py)
#
# The remote bind is loopback-only on the eval host, so nothing on the eval host' network can reach
...
# ExitOnForwardFailure: die loudly if the eval host' :8012 loopback is already taken,
...
if ($proc.HasExited) {
    Write-Error ("[brain-tunnel] ssh exited immediately (code {0}). Check ssh connectivity to {1} and whether the eval host 127.0.0.1:{2} is already bound." -f $proc.ExitCode, $SshHost, $Port)
    exit 1
}
