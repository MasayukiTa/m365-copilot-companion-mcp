...
def goal_text(lib, wt, ps):
    ...
    if os.environ.get("SWE_STRONG_SELFTEST") == "1":
        _t += ("\n\n【自己検証＝red→green を徹底】まず**編集する前に**、問題文の症状を再現する小さな"
               ...
               "実行し回帰が無いことを確認する。\n"
               "再現は**問題文に書かれた通りのオブジェクト・入力・シグネチャ（リテラルなクラス名・引数・呼び出し"
               "形）で組み立て**よ。似て非なる代用テストではなく、報告されたそのシナリオを動かすこと。さらに"
               "**期待される完全な出力をアサート**せよ——『例外が出ない』『N passed』では不十分で、正しい値・"
               "文字列・形まで突き合わせる。\n"
               "再現が通っても、データが流れる**全ての地点（生成→整形→消費 / producer→formatter→consumer）"
               "を漏れなく編集していなければ完全性は証明できていない**。報告されたシナリオを実際には動かさない"
               "既存のグリーンなテスト群は、何の証拠にもならない。\n"
