"""Phase 2 of the decoupled run: BATCH-grade captured diffs on the kiyus eval host.

Reads preds_solve/<inst>.json (written by swe_solve_decoupled.py), and for each instance ships
its diff to kiyus, runs the OFFICIAL swebench Docker eval there (kiyus has 503 GB free, so no
local disk wall), and records the verdict. Grades CONCURRENTLY (kiyus is beefy) and is resumable
...
def launch_grade(inst, diff, runid):
    """Stage the diff on kiyus + launch grade_runner detached. Returns True if launched."""
