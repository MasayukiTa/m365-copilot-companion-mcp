...
on kiyus. Shallow fetch of the exact base_commit keeps each worktree small (one machine has only
...
def goal(inst, wt):
    ...
    text = (
        ...
        "5) Self-verify EACH checklist item (named symbols importable AND actually raised/returned; each behavioral "
        "assertion traced through the code). Run the project's own tests if available (e.g. `%s`); avoid real sleep/IO "
        "in retry/poll loops. Iterate until every item holds.\n"
        "6) Then DONE. (Hidden acceptance tests are graded OFFLINE; you will not see them.)\n"
