"""DECOUPLED SWE-bench solve orchestrator (solve locally, grade later on the eval host).
...
on the the eval host eval host (bench/swe_grade_batch.py).
    ...
    for t in ordered:
        ...
        if os.path.isfile(p):
            try:
                ...
                pass
    log("=== solve done/paused: captured %d/%d (non-empty diffs: %d). Next: grade on the eval host "
