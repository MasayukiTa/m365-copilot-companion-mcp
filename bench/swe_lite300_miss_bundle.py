...
Inputs are the two official kiyus batch result files:
...
When `--pull-logs` is set, it also pulls `report.json` and `test_output.txt` from kiyus.
"""
import argparse
import collections
import json
...
def pull_logs(misses):
    ...
    r = subprocess.run(["scp", "-q", "-o", "ConnectTimeout=30",
                        "shuttle-scope:/C:/wsl-setup/miss300_logs.tgz", str(local_tar)],
