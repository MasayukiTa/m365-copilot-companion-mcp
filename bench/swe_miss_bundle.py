...
# 1) pull test_output.txt + report.json for all misses from the eval host (copy WSL->windows, scp down)
pulled = {}
if runid:
    safe = {i: re.sub(r"[^A-Za-z0-9_.-]", "_", i) for i in misses}
    # copy each instance's two log files to the windows-shared staging on the eval host
    parts = []
