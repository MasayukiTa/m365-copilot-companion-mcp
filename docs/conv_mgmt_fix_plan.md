...
1. **Bridge robustness (linchpin).**
   - `start_bridge.ps1` must reliably bring up :9223 and KEEP it alive (supervisor/scheduled task,
     analogous to kiyus SweDockerd). Initial SSO login is interactive → FOREGROUND
...
- Rebuild `ui/CopilotChat.exe` and `ui/FleetCockpit` as needed (Windows; if shuttlescope-style
