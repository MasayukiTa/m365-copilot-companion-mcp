...
- **既定で完全にバックグラウンド（SW_HIDE）起動**します。タスクバーにも Alt-Tab にも出ず、
  オレンジ点滅もしません。CDP はウィンドウが見えなくても駆動でき、スロットリング防止フラグ付き
  なので隠れていてもエージェントは鈍りません。ユーザーは Edge を一切気にせず、チャット／
  コックピットにタスクを打ち込めます。
- **サインインが必要なとき**（SSO 失効など）は自動で前面に出ます（フリート・ブリッジ両方が
  ログイン画面を検知して `surface`）。手動で出すには `.\start_companion_edge.ps1 -Surface`。
  隠したくない場合は `-Foreground`。
