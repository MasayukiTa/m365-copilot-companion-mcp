...
- **完全バックグラウンドは `-Headless`（推奨・実機検証済）**:
  `.\start_companion_edge.ps1 -Headless`
  `--headless=new` で**ウィンドウ自体が存在しません**（IsWindowVisible=False／タスクバー無し／
  点滅無し／フォアグラウンド干渉ゼロ）。それでいて **CDP・SSO 自動サインイン・送信すべて動作**
  します（検証済）。ユーザーは Edge を一切見ず・触らず、チャット／コックピットに打ち込むだけ。
  選んだモードは記憶され、`-HardReset`／自動リカバリも同じモードで再起動します。
  - 補足: 過去に試した SW_HIDE はタブのレンダラが破棄されて落ち、最小化はオレンジ点滅＋
    タスクバー、仮想デスクトップはタスクバーアイコンでテレポート——いずれも難があり、**headless が
    唯一きれいに成立**しました。
- **既定は `-Foreground`（可視・安定）**。初回サインインや、稀に SSO 失効で対話ログインが要る
  ときに使います。headless 運用中にログインが必要になったら自動で headed に再起動します
  （`surface` がモードを見て切替）。手動なら `-Foreground` で起動してサインイン→再度 `-Headless`。
