...
Started 2026-09-13. **93 → 86** so far. This file exists so the remaining 86 do not have to be
...
| `scripts/collect_lens_corpus.py::all_inconclusive` | the discard gate at `:477` still checks only `all_unclear` — "a panel where all three reviewers answered honestly was the one shape being deleted" is still live |
| `relay/outcomes.py::is_retryable` | `relay_fleet.py:6377,6379,6414` do raw `in RETRYABLE_OUTCOMES`, reintroducing the silent-default-to-False that `outcomes.py` exists to close |
| `relay/conv_title.py::salvageable` / `disambiguate` | `fleet_runner._register_convs` calls `make_title` per row and never de-duplicates, so the module's whole purpose (213 of 424 titles mass-produced) never runs |
...
| `relay/transport_policy.py::duplicate_risk` | `relay_fleet.py:5231` inlines its body; a test even records that this path "neither referenced nor recorded it" |
...
| `scripts/stale_server_check.py::fleet_is_running` | bypassed by a duplicate inline `_run_appears_live` in the same file |
