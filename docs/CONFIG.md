...
| `MCP_ALLOWED_BASE` | エージェントがアクセスできるフォルダの上限。未設定/空 = ホーム全体（fail-closed）。`~/work` 等に絞ると隔離が固くなる。`*` はホームの制限を外し**全ドライブ**を許可する明示的オプトイン（74989c3 以降） | 未設定 = ホームのみ |
...
## `MCP_ALLOWED_BASE=*`（全ドライブを許可する明示的オプトイン）

- 未設定・空 = ホームディレクトリのみ（`tools/file_ops.py::_parse_allowed_bases` の既定、fail-closed）。
- `MCP_ALLOWED_BASE=*` にすると `tools.file_ops.ALLOWED_BASES` が `None` になり、読み書きツールが**全ドライブ**にアクセスできるようになります。旧バージョン（74989c3 より前）で「未設定＝無制限」だった挙動に依存していたインストールを引き継ぐための値で、新規インストールでは通常不要です。
- `;`（Windows のパス区切り）で複数ルートを列挙することもできます（例: `C:/;D:/` や `~;D:/data`）。`*` はその中の唯一の「無制限」値で、それ以外の値はすべて列挙されたルート配下に制限されます。
